...
#     return ((np.cosh(2./kappa))**2/((np.sinh(2./kappa))**4)*((np.sinh(2./kappa))**2-1.))**2
#     return ((1.+kappa*kappa)/((1.-kappa*kappa)**2)*((1.-6.*kappa*kappa+kappa**4)**0.5))
     return (1-(np.sinh(2./kappa/2.24)**(-4)))**0.125
...
for i in range(n):
  kappa = float(i)*dt
  status, current_energy,H_avg,M_avg = Monte_Carlo(status,current_energy,J,kappa*TC_square)
  print(H_avg)
  y1[i] = H_avg
  y2[i] = M_avg
  y2_thesis[i] = thesis(kappa)
  if(i > 0):
     y3[i] = (y1[i]-y1[i-1])/(dt*TC_square)
...
for i in range(n):
  kappa = float(i)*dt
  status, current_energy,H_avg,M_avg = Monte_Carlo_honeycube(status,current_energy,J,kappa*TC_honeycube)
  y1_1[i] = H_avg
  y2_1[i] = M_avg
  print(H_avg)
  if(i > 0):
     y3_1[i] = (y1_1[i]-y1_1[i-1])/(dt*TC_honeycube)
